...
from model_EffNet import EfficientNetV2Classifier
...
from easy_to_hard import compute_confidence_scores, group_by_confidence, curriculum_dataloader
...
import numpy as np
import random
from visualize_f101 import visualize_attr_maps
...
class ExplainabilityTracker:
    ...
    def update(self, current_score):
        """
        Updates the tracker with the latest explainability score.
        Args:
...
def denormalize(image, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    mean = torch.tensor(mean).view(3, 1, 1)  # Shape (C, 1, 1) to match image dimensions
    std = torch.tensor(std).view(3, 1, 1)    # Shape (C, 1, 1)
    if image.dim() == 4:  # Batch of images (B, C, H, W)
        mean = mean.unsqueeze(0)  # Shape: (1, C, 1, 1)
        std = std.unsqueeze(0)    # Shape: (1, C, 1, 1)


    image = image * std + mean  # Reverse normalization
    return image.clamp(0, 1)  # Ensure the image values are in the range [0, 1]
...
def log_fixed_images_with_dropout(original_images, dropout_images, epoch):
    for i in range(len(original_images)):
        ...
        original_image = denormalize(original_images[i].cpu().detach()).numpy().transpose(1, 2, 0)  # CHW -> HWC
        dropout_image = denormalize(dropout_images[i].cpu().detach()).numpy().transpose(1, 2, 0)  # CHW -> HWC

        # Plot original and dropout-applied images
...
def log_attributions(attributions, images, epoch, method_name="Attribution"):
    for i in range(len(images)):
        # Convert image and attribution to numpy for visualization
        image = denormalize(images[i].cpu().detach()).numpy().transpose(1, 2, 0)  # CHW -> HWC for RGB
        attribution = attributions[i].cpu().detach().numpy()
        
        # Aggregate attribution map across channels (if multi-channel)
        if attribution.shape[0] == 3:  # For color images
            attribution = np.mean(attribution, axis=0)  # Take mean over the channels to get a 2D map
...
def main():
    #set_seed(24)
    # Argument parser to pass configuration dynamically
    ...
    scheduler = CurriculumScheduler(
        initial_theta=1.0, final_theta=0.5, warmup_epochs=30, total_epochs=args.epochs, gamma=0.001, decay_type='exponential',
        metric_type= args.scheduler
    )
  # Pass selected scheduler type
    ...
    best_val_acc = 0.0
    model_name = f"{args.dataset}_{args.model}_{args.explainer}_{args.scheduler}_sufficiency.pt"
    ...
    for i, (img, label) in enumerate(train_loader):
        if len(fixed_images) < 3:
            ...
            fixed_labels.append(label[0].clone().detach().unsqueeze(0))
        else:
            break

    fixed_images = torch.cat(fixed_images, dim=0).to(device)  # Stack them together
    ...
    for epoch in range(args.epochs):
        ...
        if args.scheduler == 'easyhard':
            ...
            train_loader = curriculum_dataloader(epoch, args.epochs, original_train_dataset, train_loader,
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            if args.scheduler == 'easyhard':
                explainability_score = evaluator.sufficiency(images, attributions)
                ...
                infidelity_count += 1
                explainability_tracker.update(explainability_score.mean().item())
            ...
            if args.scheduler == 'easyhard':
                # Do not apply any dropout for the 'easyhard' scheduler
        ...
        if args.scheduler == 'easyhard':
            explainability_tracker.update(avg_sufficiency_score)
        ...
        if val_acc > best_val_acc:
            ...
            early_stopping_counter += 1  # No improvement in validation accuracy
            if early_stopping_counter >= patience:
                print(f"Early stopping triggered. No improvement in {patience} epochs.")
                break  # Stop training early if no improvement is seen for `patience` epochs
        ...
        # Log metrics for each epoch during training and validation
        wandb.log({
            "epoch": epoch + 1,
    ...
    # After training, load the best model and evaluate on the test dataset
