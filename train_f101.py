  ...
  # Pass selected scheduler type
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            # 1. Select explainer dynamically based on the command-line argument
            ...
            if args.scheduler == 'epoch':
                ...
                model.update_dropout_rate(epoch, args.epochs)
            else:
                # Calculate explainability score for the current batch
                explainability_score = None
                attributions = None
                if args.explainer == 'saliency':
                    attributions = explainability.compute_saliency(images, target=labels)
                ...
                # Get the explainability score for the scheduler
                if args.scheduler == 'sufficiency':
                    explainability_score = explainability.calculate_sufficiency(images, attributions)
                elif args.scheduler == 'infidelity':
                    explainability_score = infidelity(model, perturb_fn, images, attributions)
                elif args.scheduler == 'sensitivity':
                    explainability_score = sensitivity_max(explainability.compute_saliency, images, target=labels)

                # Update dropout rate based on explainability score
                if explainability_score is not None:
                    model.update_dropout_rate(explainability_score, args.epochs)
              """
