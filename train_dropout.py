...
class ExplainabilityTracker:
    ...
    def update(self, current_score):
        ...
        if self.epochs_without_improvement >= self.patience:
            print(f"No improvement in explainability for {self.patience} epochs.")
...
def main():
    ...
    wandb.init(project="cl_exp", entity="gizem-mert", name=run_name, config=args)
    ...
    for epoch in range(args.epochs):
        ...
        total_explainability_score = 0.0 #for the xAI scheduler of dropout
        dropout_probability = 1.0
        if epoch < scheduler.warmup_epochs:
            print(f"Warmup period: Skipping dropout for epoch {epoch}.")
            apply_dropout_next_epoch = False
        else:
            # After warmup, determine if dropout should be applied based on explainability tracker
            apply_dropout_next_epoch = explainability_tracker.epochs_without_improvement < explainability_tracker.patience


        # Track if dropout should be applied for the next epoch
        ...
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            if args.scheduler != 'epoch':
                if args.scheduler == 'sufficiency':
                    ...
                    sufficiency_count += len(images_subsample)
                elif args.scheduler == 'infidelity':
                    explainability_score = explainability.calculate_infidelity(perturb_fn, images_subsample, attributions,
                                                                               target=labels_subsample)
                    infid_score_sum += explainability_score.item()
                    infidelity_count += 1
                elif args.scheduler == 'sensitivity':
                    explainability_score = explainability.calculate_sensitivity(explainer_method, images_subsample,
                                                                                target=labels_subsample)
                    sens_score_sum += explainability_score.item()
                    sensitivity_count += 1

            if explainability_score is not None:
                total_explainability_score += explainability_score


            if apply_dropout_next_epoch:
                # If the score is improving, apply dropout
                if args.scheduler == 'epoch':
                    ...
                    images = scheduler.apply_dropout(images,
                                                     explainability_score=explainability_score)  # Explainability-based dropout
        ...
        if epoch >= scheduler.warmup_epochs:
            avg_explainability_score = total_explainability_score / len(train_loader)
            explainability_tracker.update(avg_explainability_score)
        ...
        wandb.log({
            ...
            "samples_explained": sufficiency_count + infidelity_count + sensitivity_count,
    ...
    with torch.no_grad():
        for test_images, test_labels in test_loader:
            ...
            if args.explainer == 'saliency':
                ...
                attributions = explainability.compute_guided_backprop(images, target=test_labels)
                explainer_method = explainability.compute_guided_backprop
            elif args.explainer == 'compute_input_x_gradient':
